...
describe('BinPar Framework types coverage', () => {
  test('Basic', () => {
    ...
    expect(types.object.graphQLType({ entityShortName: 'Test' }, null, '')).toBe('Test');
    expect(check(String, types.phone.getFakedValue())).toBe(true);
    expect(check(String, types.string.getFakedValue())).toBe(true);

    expect(
      types.oneToManyReference.graphQLType(
        { targetCollectionShortName: 'References' },
        [
          {
            collectionShortName: 'References',
            entityShortName: 'Reference',
          },
        ],
        '',
      ),
    ...
    expect(
      types.oneToManyReference.mongooseRef(
        { targetCollectionShortName: 'References' },
        [
          {
            collectionShortName: 'References',
