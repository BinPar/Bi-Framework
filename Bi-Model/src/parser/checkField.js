...
export default function checkField(fieldName, field, shortName) {
  ...
  if (properties.indexOf('type') === -1) {
    throw new Error(`Field "${fieldName}" must have the property "type" in "${shortName}"`);
    ...
    throw new Error(
      `Property "type" of "${fieldName}" must be a valid BinPar Framework Type which must have a property "requiredProperties" => in "${shortName}"`,
    );
  }
  for (let j = 0, l2 = properties.length; j < l2; j += 1) {
    ...
    if (ALLOWED_PROPERTIES.indexOf(propertyName) === -1) {
      throw new Error(
        `Property "${propertyName}" is not a valid BinPar Framework property of field "${fieldName}" => in "${shortName}"`,
      );
    }
  ...
  for (let j = 0, l2 = requiredProperties.length; j < l2; j += 1) {
    ...
    if (properties.indexOf(requiredProperty.propName) === -1) {
      throw new Error(
        `The type of field "${fieldName}" must have the property "${requiredProperty.propName}" => in "${shortName}"`,
      );
    }
    ...
    if (!check(requiredParsedType, propVal)) {
      throw new Error(
        `The type of field "${fieldName}" must be "${requiredParsedType}" => in "${shortName}"`,
      );
    }
  }
  return true;
